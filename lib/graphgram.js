...
Grammar.prototype.evolve = function (opts) {
  ...
  if (opts.graph) {
    ...
    context.warn (colors.yellow ("Initial grammar has " + graph.nodes().length + " nodes, " + graph.edges().length + " edges"))
    context.findNextId()
  } else {
...
function Context (json, opts) {
  extend (this, json, { nextId: 1 })
  this.warn = this.grammar.warn
  this.verbose = this.grammar.verbose
  extend (this, opts)
}

Context.prototype.findNextId = function() {
  this.graph.nodes().forEach (function (id) {
    var n = parseInt (id)
    if (n) this.nextId = Math.max (this.nextId, n + 1)
  })
}
