                    ...
                    oneOf: [{ type: 'string' },  // interpreted as a label
                            ...
                            { type: 'object',
                              ...
                              properties: extend ({
                                ...
                                label: { type: 'string' }  // lhs labels interpreted as regexes, rhs labels expanded using ${id.label} or ${id.match[n]} for n'th group
                              }, lhs ? {
...
Grammar.prototype.schema = {
  ...
  additionalProperties: false,
  properties: {
    start: { type: 'string' },  // starting node
    limit: { type: 'number' },  // maximum number of rule applications
    induced: { type: 'boolean' },  // default 'induced', overridden by 'induced' for individual rules
    rules: {
      type: 'array',
      items: {
        type: 'object',
...
Grammar.prototype._init = function (errorCallback) {
  ...
  this.rules.forEach (function (rule, n) {
    ...
    Object.keys(headOrTail).forEach (function (id) {
      if (isRhsNodeId[id])
        throw new Error ("In " + ruleName + ": lhs node " + id + " is listed as " + headOrTail[id] + " for an rhs node, but is also an rhs node ID")
    })
...
function labelMatch (gLabel, sLabel) {
  if (!sLabel) return [gLabel]
  var match = new RegExp('^'+sLabel+'$').exec (gLabel)
  return match && match.slice(0)
}
  ...
  for (var iter = 0; typeof(limit) === 'undefined' || iter < limit; ++iter) {
    ...
    this.rules.forEach (function (rule, n) {
      ...
      isomorphs.forEach (function (isomorph) {
        ...
        if (!mismatch && context.evalCond (isomorph, rule)) {
	  ...
	  isomorph.label = {}
	  rule.lhs.node.forEach (function (node) {
	    isomorph.label[node.id] = graph.node (isomorph.assign[node.id])
	  })
          if (rule.lhs.edge)
...
Grammar.prototype.toDot = function (graph) {
  return ["digraph G {"]
    .concat (graph.nodes().map (function (id) {
      return '  ' + id + ' [label="' + graph.node(id) + '"];'
    })).concat (graph.edges().map (function (edge) {
      var label = graph.edge(edge)
      return '  ' + edge.v + ' -> ' + edge.w + (label ? (' [label="' + label + '"]') : '') + ';'
    })).concat (['}',''])
    .join("\n")
}
