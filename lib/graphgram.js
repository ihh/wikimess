        ...
        node: {
          ...
          items: {
            ...
            properties: {
              ...
              head: { type: 'array', items: { "$ref": "#/definitions/identifier" } },
              tail: { type: 'array', items: { "$ref": "#/definitions/identifier" } },
              label: { type: 'string' }
            }
          }
        },
...
Grammar.prototype._init = function (errorCallback) {
  ...
  this.rules.forEach (function (rule, n) {
    ...
    function checkGraph (graph, name, isLhsNodeId) {
    ...
    var isLhsNodeId = checkGraph (rule.lhs, 'rule #' + n + ' lhs')
    checkGraph (rule.rhs, 'rule #' + n + ' rhs', isLhsNodeId)
...
Grammar.prototype.evolve = function (opts) {
  ...
  for (var iter = 0; typeof(limit) === 'undefined' || iter < limit; ++iter) {
    ...
    this.rules.forEach (function (rule, n) {
      ...
      var isomorphs = grammar.subgraphIsomorphisms (graph, rule.lhsGraph)
  ...
  if (nAssigned) {
    ...
    subgraph.edges().forEach (function (edge) {
      if (edge.v <= lastAssigned && edge.w <= lastAssigned) {
        var match = grammar.testEdgeMatch (graph, mapping.assign[edge.v], mapping.assign[edge.w], edge.label)
        if (!match)
          edgeNotFound = true
        else if (edge.id)
          edgeMatch[edge.id] = match
      }
    })
    if (edgeNotFound)
      return []
