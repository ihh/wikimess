...
function labelMatch (gLabel, sLabel) {
  if (typeof(sLabel) === 'undefined')
    return { match: gLabel }
  if (typeof(sLabel) !== typeof(gLabel))
    return false
  if (typeof(sLabel) === 'string') {
    var match = new RegExp('^'+sLabel+'$').exec (gLabel)
    return match && { match: match.slice(0) }
  } else if (isArray(sLabel)) {
    if (sLabel.length !== gLabel.length)
      return false
    var allMatch = true
    ...
    if (sLabel[globalKey])
      allMatch = allMatch && Object.keys(sLabel).length == Object.keys(gLabel).length + 1
    Object.keys(sLabel).forEach (function (k) {
      if (allMatch && k !== globalKey) {
    ...
    return gLabel === sLabel && { match: gLabel }
}
