...
Grammar.prototype.evolve = function (opts) {
  ...
  for (var iter = 0; typeof(limit) === 'undefined' || iter < limit; ++iter) {
    ...
    this.rules.forEach (function (rule, n) {
      ...
      if (typeof(lhs) === 'string') {
	    ...
	    var ids = [edge.v, edge.w]
            if (context.evalCond (match, ids, edge.name, rule))
    ...
    var newLabels = site.rhs.node.map (function (labelExpr) {
      return context.newLabel (site.match, site.lhs, site.edge && site.edge.name, labelExpr)
    })
    ...
    site.rhs.edge.forEach (function (edge) {
      var label = context.newLabel (site.match, site.lhs, site.edge && site.edge.name, edge[2])
...
Context.prototype.addSite = function (rule, lhs, rhs, match, edge) {
  var weight = this.evalWeight (match, lhs, edge && edge.name, rule)
...
Context.prototype.newLabel = function (match, ids, edgeId, expr) {
  if (!expr) return expr
  if (expr.match(/^=/))
    return this.evalMatchExpr (match, ids, edgeId, expr.replace(/^=/,''))
  return expr.replace (/\\(\d+)/g, function (_m, n) { return match[parseInt(n)] })
}

Context.prototype.evalCond = function (match, ids, edgeId, rule) {
  return this.evalMatchExpr (match, ids, edgeId, rule.condition, true)
}

Context.prototype.evalWeight = function (match, ids, edgeId, rule) {
  return this.evalMatchExpr (match, ids, edgeId, rule.weight, 1)
}

Context.prototype.evalMatchExpr = function (match, ids, edgeId, expr, defaultVal) {
  var context = this
