  ...
  for (var iter = 0; typeof(limit) === 'undefined' || iter < limit; ++iter) {
    ...
    this.rules.forEach (function (rule, n) {
      ...
      isomorphs.forEach (function (isomorph) {
        var degreeMismatch = false
        rule.lhs.node.forEach (function (node) {
          degreeMismatch = degreeMismatch || (node.strict && rule.lhsGraph.neighbors(node.id).length != graph.neighbors(isomorph.assign[node.id]).length)
        })
        if (!degreeMismatch && context.evalCond (isomorph, rule)) {
