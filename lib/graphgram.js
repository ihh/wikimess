  ...
  this.rules.forEach (function (rule, n) {
	...
	graph.node = graph.node.map (function (node, n) {
	  ...
	  if (isArray(node))
	    ...
	    if (typeof(node.label) === 'undefined' && lhsInfo && lhsInfo.isNodeId[node.id])
	      node.label = node.assign
	      ? grammar.matcher.makeLabelAssign (node.id, node.assign)
	...
	if (graph.edge)
		...
		if (typeof(e.label) === 'undefined')
		  e.label = e.assign
		  ? grammar.matcher.makeLabelAssign (e.id, e.assign)
...
Matcher.prototype.makeLabelAssign = function (id, update) {
  var result = {}
