...
Grammar.prototype.evolve = function (opts) {
  ...
  var initContext = new Context ({ grammar: this }, opts)
  ...
  if (opts.graph) {
    graph = opts.graph
    initContext.warn (colors.yellow ("Initial grammar has " + graph.nodes().length + " nodes, " + graph.edges().length + " edges"))
    ...
    initContext.warn (colors.yellow ("Initializing grammar with " + grammar.start))
  }
  ...
  for (var iter = 0; typeof(limit) === 'undefined' || iter < limit; ++iter) {
    ...
    var context = new Context ({ iter, graph, grammar, sites }, opts)
    ...
    //  context.warn (colors.yellow (JSON.stringify (graphlib.json.write (graph))))
  }
