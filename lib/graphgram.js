...
Grammar.prototype.makeGraphSchema = function (canonical, lhs) {
...
Grammar.prototype.makeSchema = function (canonical) {
  return {
    ...
    definitions: {
      ...
      rules: {
        ...
        items: {
          ...
          properties: {
	    name: { type: 'string' },
            lhs: this.makeGraphSchema(canonical,true),
            rhs: this.makeGraphSchema(canonical,false),
...
Grammar.prototype.validate = function (json) {
  ...
  var schema = this.makeSchema (this.canonical)
//  console.log (JSON.stringify (schema, null, 2)); process.exit()
  ...
  this.rules.forEach (function (rule, n) {
	...
	if (isArray(node))
	  ...
	  if (typeof(node.label) === 'undefined' && lhsInfo && lhsInfo.isNodeId[node.id])
            node.label = grammar.matcher.makeLabelEval (node.id)
	  return node
	}
	return { id: nodeIdFactory(n), label: node }
	  ...
	  if (e.id && lhsInfo && lhsInfo.isEdgeId[e.id]) {
	      ...
	      e.label = grammar.matcher.makeLabelEval (e.id)
...
function Context (json, opts) {
  ...
  this.ruleCount = {}
  this.grammar.rules.forEach ((rule) => { this.ruleCount[rule.type] = 0 })
...
Context.prototype.updateIteration = function (iter) {
  ...
  this.warn (colors.cyan ("Iteration " + (iter+1) + " of " + this.grammar.name))
}
...
Context.prototype.sampleRuleSite = function() {
  ...
  grammar.rules.forEach (function (rule, n) {
    if (rule.limit && context.ruleCount[rule.type] >= rule.limit)
      return
  ...
  context.warn(colors.blue("Total weight is " + totalWeight + "; " + (totalWeight > 0 ? ("sampled " + sites[m].rule.name) : "no rules to apply")))
