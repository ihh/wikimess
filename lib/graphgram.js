...
Context.prototype.newLabel = function (match, ids, edge, expr) {
  if (!expr) return expr
  if (expr.match(/^=/))
    return this.evalMatchExpr (match, ids, edge, expr.replace(/^=/,''))
  return expr.replace (/\\(\d+)/g, function (_m, n) { return match[parseInt(n)] })
}

Context.prototype.evalCond = function (match, ids, edge, rule) {
  return this.evalMatchExpr (match, ids, edge, rule.condition, true)
}

Context.prototype.evalWeight = function (match, ids, edge, rule) {
  return this.evalMatchExpr (match, ids, edge, rule.weight, 1)
}

Context.prototype.evalMatchExpr = function (match, ids, edge, expr, defaultVal) {
  var context = this
  ...
  if (ids.length == 2) {
    ...
    extendedContext.edge = edge
  } else
    extendedContext.node = ids[0]
